...
def test_parse_date_today():
    ...
    result = parse_date("today", current_date=mock_now)
    assert result == "2026-03-11"


def test_parse_date_tomorrow():
    mock_now = datetime(2026, 3, 11, 14, 0, 0)
    result = parse_date("tomorrow", current_date=mock_now)
...
def test_parse_date_next_weekday_same_day():
    ...
    result = parse_date("wednesday", current_date=mock_now)
...
def test_parse_date_next_weekday_future():
    ...
    result = parse_date("friday", current_date=mock_now)
...
def test_parse_date_next_weekday_past():
    ...
    result = parse_date("monday", current_date=mock_now)
    assert result == "2026-03-16"


def test_parse_date_dd_mm():
    mock_now = datetime(2026, 3, 11, 14, 0, 0)
    result = parse_date("15-08", current_date=mock_now)
